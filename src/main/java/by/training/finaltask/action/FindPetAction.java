        ...
        if (session != null) {
            ...
            if (pets == null && petPicturePaths == null) {
                ...
                if (user != null && allowedRoles.contains(user.getUserRole())) {
                    pagination = new Pagination(petService.getAllCountByStatus(null),
                            ROWCOUNT, request.getParameter("page"));
                    pets = petService.getAllByStatus(null, pagination.getOffset(), ROWCOUNT);
                } else {
                    pagination = new Pagination(petService.getAllCountByStatus(PetStatus.SHELTERED)
                            , ROWCOUNT, request.getParameter("page"));
                    pets = petService.getAllByStatus(PetStatus.SHELTERED, pagination.getOffset(), ROWCOUNT);
